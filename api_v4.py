...
def search_in_database(
    ...
    """Busca em um banco de dados específico usando CONTATOS_ID, DocNumber ou campo customizado"""
    ...
    try:
        ...
        if search_by_field:
            search_field = search_by_field
        elif search_by_cpf and db_name == "contatos":
            search_field = "CPF"
        else:
            search_field = key_field
        
        query = f"SELECT * FROM {table} WHERE {search_field} = ? LIMIT ?"
...
async def root():
    ...
    return {
        ...
        "endpoints": {
            ...
            "/telefone/{telefone}": "Busca por telefone (com ou sem DDD)",
            "/webhook/telefone": "Webhook para busca por telefone (POST)",
...
@app.get("/telefone/{telefone}")
async def search_telefone(
    telefone: str,
    ...
    Parâmetros:
    - telefone: Número do telefone (com ou sem DDD, com ou sem formatação)
    - limit: Limite de resultados (1-1000, padrão 100)
    
    Exemplos:
    - /telefone/11962990723
    - /telefone/(11) 96299-0723
    - /telefone/962990723
    """
    ...
    try:
        ...
        if len(telefone_clean) < 8:
            raise HTTPException(status_code=400, detail="Telefone deve ter pelo menos 8 dígitos")
        
        # Buscar no histórico de telefones
        historico_result = search_in_database(
            "historico_telefones",
            DATABASES["historico_telefones"],
            telefone_clean,
            limit,
            search_by_field="TELEFONE"
        )
        
        results = {"historico_telefones": historico_result}
        
        # Se encontrou telefones, buscar dados completos dos contatos
        if historico_result["status"] == "success" and historico_result["count"] > 0:
            # Extrair CONTATOS_IDs únicos
            contatos_ids = set()
            for record in historico_result["data"]:
                if "CONTATOS_ID" in record:
                    contatos_ids.add(str(record["CONTATOS_ID"]))
            
            # Buscar dados de cada contato em paralelo
            with ThreadPoolExecutor(max_workers=4) as executor:
                futures = {}
                
                for contatos_id in list(contatos_ids)[:10]:  # Limitar a 10 contatos
                    for db_name in ["contatos", "poder_aquisitivo", "irpf"]:
                        db_config = DATABASES[db_name]
                        future = executor.submit(
                            search_in_database,
                            db_name,
                        ...
                        futures[future] = (db_name, contatos_id)
                
                # Organizar resultados por CONTATOS_ID
                contatos_data = {}
                for future in as_completed(futures):
                    db_name, contatos_id = futures[future]
                    try:
                        result = future.result()
                        if contatos_id not in contatos_data:
                            contatos_data[contatos_id] = {}
                        contatos_data[contatos_id][db_name] = result
                    except Exception as e:
                        logger.error(f"Erro ao buscar {db_name} para {contatos_id}: {e}")
                
                results["contatos_data"] = contatos_data
        ...
        return {
            ...
            "search_value": telefone,
            "search_value_normalized": telefone_clean,
            "databases": results,
            "summary": {
                "total_databases_queried": len(results),
                "successful_queries": sum(1 for r in results.values() if isinstance(r, dict) and r.get("status") == "success"),
                "failed_queries": sum(1 for r in results.values() if isinstance(r, dict) and r.get("status") == "error"),
                "total_records": sum(r.get("count", 0) if isinstance(r, dict) else 0 for r in results.values())
            },
            "consistency": consistency,
            "from_cache": False,
        ...
        logger.error(f"Erro em /telefone: {e}")
        raise HTTPException(status_code=500, detail=f"Erro interno: {str(e)}")

from pydantic import BaseModel

class TelefoneWebhookRequest(BaseModel):
    """Modelo para requisição webhook de telefone"""
    telefone: str
    limit: int = 100

@app.post("/webhook/telefone")
async def webhook_telefone(request: TelefoneWebhookRequest):
    """
    Webhook para busca por telefone (POST)
    ...
    Retorna todos os dados cruzados associados ao telefone.
    """
    
    try:
        # Normalizar telefone
        telefone_clean = re.sub(r'\D', '', request.telefone)
        
        if len(telefone_clean) < 8:
            raise HTTPException(status_code=400, detail="Telefone deve ter pelo menos 8 dígitos")
        
        # Chamar endpoint de telefone
        return await search_telefone(request.telefone, request.limit)
    
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Erro em webhook /telefone: {e}")
        raise HTTPException(status_code=500, detail=f"Erro interno: {str(e)}")
